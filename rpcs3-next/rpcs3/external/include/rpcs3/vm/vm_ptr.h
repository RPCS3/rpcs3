...
namespace rpcs3
{
	...
	namespace vm
	{
		...
		public:
			...
			RT operator()(rpcs3::cell::ppu::thread& CPU, T... args) const;

			// defined in ARMv7Callback.h, passing context is mandatory
			RT operator()(rpcs3::psv::armv7::thread& context, T... args) const;
...
namespace common
{
	namespace fmt
	{
		...
		template<typename T, typename AT> struct unveil<rpcs3::vm::_ptr_base<T, AT>, false>
		{
			using result_type = typename unveil<AT>::result_type;

			static inline result_type get_value(const rpcs3::vm::_ptr_base<T, AT>& arg)
			{
