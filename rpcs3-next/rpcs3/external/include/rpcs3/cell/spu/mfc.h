...
namespace rpcs3
{
	...
	namespace mfc
	{
		...
		{
			WrMSSyncReq = 9,  // Write multisource synchronization request
			RdTagMask = 12, // Read tag mask
			LSA = 16, // Write local memory address command parameter
			EAH = 17, // Write high order DMA effective address command parameter
			EAL = 18, // Write low order DMA effective address command parameter
			Size = 19, // Write DMA transfer size command parameter
			TagID = 20, // Write tag identifier command parameter
			Cmd = 21, // Write and enqueue DMA command with associated class ID
			WrTagMask = 22, // Write tag mask
			WrTagUpdate = 23, // Write request for conditional or unconditional tag status update
			RdTagStat = 24, // Read tag status with mask applied
			RdListStallStat = 25, // Read DMA list stall-and-notify status
			WrListStallAck = 26, // Write DMA list stall-and-notify acknowledge
			RdAtomicStat = 27, // Read completion status of last completed immediate MFC atomic update command
		};
