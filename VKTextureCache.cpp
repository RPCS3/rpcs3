...
void texture_cache::release_temporary_subresource(vk::image_view* view)
{
    if (!view || !view->handle)
    {
        ppu_log.warning("release_temporary_subresource: Attempted to release a null or invalid image view");
        return;
    }

    // Log the release for debugging purposes
    ppu_log.debug("Releasing temporary Vulkan image view: {}", view->handle);

    // Destroy the Vulkan image view
    vkDestroyImageView(m_device, view->handle, nullptr);
    view->handle = VK_NULL_HANDLE;  // Invalidate the handle
...
void texture_cache::release_temporary_subresource(std::unique_ptr<vk_image_view_wrapper>& view)
{
    if (!view || view->m_view == VK_NULL_HANDLE)
    {
        ppu_log.warning("release_temporary_subresource: Attempted to release a null or invalid image view");
        return;
    }

    // Automatically clean up resources when the unique_ptr is reset or goes out of scope
    ppu_log.debug("Releasing Vulkan image view: {}", view->m_view);
    view.reset(); // Reset the unique_ptr to invoke the RAII destructor
}

// Refactor texture cache cleanup using RAII
void texture_cache::destroy()
{
    ppu_log.debug("Destroying texture cache and releasing resources");

    // Release all cached texture sections
    for (auto& section : m_cached_sections)
    {
        on_section_destroyed(section);
    }
    m_cached_sections.clear();

    // Purge unused temporary resources
    purge_unreleased_sections();

    ppu_log.info("Texture cache successfully destroyed");
}
