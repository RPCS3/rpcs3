...
bool vfs::mount(std::string_view vpath, std::string_view path, bool is_dir)
{
    if (vpath.empty())
    {
        vfs_log.error("Cannot mount empty VFS path to "%s"", path);
        return false;
    }

    // Detect and handle PS3 game discs
    if (is_ps3_disc(path))
    {
        vfs_log.info("Detected PS3 game disc at path: %s", path);
        if (!mount_ps3_disc(vpath, path))
        {
            vfs_log.error("Failed to mount PS3 disc at path: %s", path);
            return false;
        }
        return true;
    }
    ...
    const std::string_view vpath_backup = vpath;
    // Path adjustments, validations, and mount operations...
    // ...

    vfs_log.info("Successfully mounted path "%s" to "%s"", vpath_backup, path);
    return true;
}

// Helper function to detect PS3 discs
bool vfs::is_ps3_disc(std::string_view path)
{
    // Check for typical PS3 disc metadata files (e.g., PS3_GAME/LICDIR, PARAM.SFO)
    std::string ps3_game_dir = std::string(path) + "/PS3_GAME";
    return fs::exists(ps3_game_dir) && fs::is_directory(ps3_game_dir);
}

// Helper function to mount PS3 discs
bool vfs::mount_ps3_disc(std::string_view vpath, std::string_view path)
{
    // Example logic to mount PS3 discs
    std::string ps3_game_dir = std::string(path) + "/PS3_GAME";
    std::string ps3_content_dir = ps3_game_dir + "/USRDIR";

    if (!fs::exists(ps3_content_dir))
    {
        vfs_log.error("PS3 disc is missing essential directories at path: %s", path);
        return false;
    }

    // Mount the disc content directory to the VFS path
    return vfs::mount(vpath, ps3_content_dir, true);
}
...
// Helper function to mount ISO files
bool vfs::mount_iso(std::string_view vpath, std::string_view iso_path)
{
    vfs_log.info("Attempting to mount ISO file: %s", iso_path);

    // Validate the ISO file
    if (!fs::exists(iso_path) || !fs::is_regular_file(iso_path))
    {
        vfs_log.error("Invalid ISO file path: %s", iso_path);
        return false;
    }

    // Parse the ISO file (basic ISO9660 support)
    if (!parse_iso(iso_path))
    {
        vfs_log.error("Failed to parse ISO file: %s", iso_path);
        return false;
    }

    // Mount parsed contents to the VFS path
    return vfs::mount(vpath, iso_path, true);
}

// Basic ISO9660 parser (stub implementation)
bool vfs::parse_iso(std::string_view iso_path)
{
    std::ifstream iso_file(iso_path.data(), std::ios::binary);
    if (!iso_file.is_open())
    {
        vfs_log.error("Failed to open ISO file: %s", iso_path);
        return false;
    }

    // ISO parsing logic (stubbed)
    vfs_log.info("Parsing ISO file: %s", iso_path);
    // TODO: Add detailed parsing logic for ISO9660

    iso_file.close();
    return true;
}

// Helper function to parse PARAM.SFO for game metadata
bool vfs::parse_param_sfo(const std::string& param_sfo_path, std::map<std::string, std::string>& metadata)
{
    std::ifstream param_file(param_sfo_path, std::ios::binary);
    if (!param_file.is_open())
    {
        vfs_log.error("Failed to open PARAM.SFO file: %s", param_sfo_path);
        return false;
    }

    // SFO parsing logic (simplified example)
    vfs_log.info("Parsing PARAM.SFO for metadata: %s", param_sfo_path);

    // Example: Extract game title and ID
    metadata["TITLE"] = "Example Game Title"; // Replace with actual parsing logic
    metadata["GAME_ID"] = "ExampleGameID";    // Replace with actual parsing logic
