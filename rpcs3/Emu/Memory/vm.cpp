...
namespace vm
{
		...
		, flags(process_block_flags(flags))
	{
		if (this->flags & preallocated)
		{
			...
			if (!map_critical(memory_4GB_model->base_addr + addr, this->flags & block_size_4k && utils::get_page_size() > 4096 ? utils::protection::rw : utils::protection::no)
		...
		, flags(process_block_flags(ar.pop<u64>(), true, addr))
	{
		...
		{
			m_common = std::make_shared<utils::shm>(size, fmt::format("_block_x%08x", addr));
			m_common->map_critical(memory_4GB_model->base_addr + addr, this->flags & block_size_4k && utils::get_page_size() > 4096 ? utils::protection::rw : utils::protection::no);
