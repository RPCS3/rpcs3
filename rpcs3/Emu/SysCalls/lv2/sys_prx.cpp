...
{
	sys_prx.Warning("prx_load_module(path=\"%s\", flags=0x%llx, pOpt=0x%x)", path.c_str(), flags, pOpt.addr());
	...
	for (auto &module_ : info.modules)
	{
		...
		for (auto &import : module_.second.imports)
		{
			...
			else
			{
				...
				sys_prx.Error("Imported %s function '%s' in '%s' module (0x%x)", (is_lle ? "LLE " : ""), SysCalls::GetFuncName(nid), module_.first, addr);
			}
	...
	return Emu.GetIdManager().GetNewID(prx, TYPE_PRX);
}

s32 sys_prx_load_module(vm::ptr<const char> path, u64 flags, vm::ptr<sys_prx_load_module_option_t> pOpt)
{
	sys_prx.Warning("sys_prx_load_module(path=\"%s\", flags=0x%llx, pOpt=0x%x)", path.get_ptr(), flags, pOpt.addr());
...
{
	sys_prx.Warning("sys_prx_start_module(id=%d, flags=0x%llx, pOpt=0x%x)", id, flags, pOpt);
...
{
	sys_prx.Warning("sys_prx_stop_module(id=%d, flags=0x%llx, pOpt=0x%x)", id, flags, pOpt);
...
{
	sys_prx.Warning("sys_prx_unload_module(id=%d, flags=0x%llx, pOpt=0x%x)", id, flags, pOpt.addr());
