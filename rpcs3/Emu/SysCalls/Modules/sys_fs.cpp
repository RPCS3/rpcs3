...
fin:
	...
	ConLog.Warning("*** fsAioRead(fd=%d, offset=0x%llx, buf_addr=0x%x, size=0x%x, error=0x%x, res=0x%x, xid=0x%x )",
		fd, (u64)aio->offset, buf_addr, (u64)aio->size, error, res, xid);

	ConLog.Warning("*** fsAioRead([%s])", path.c_str());
