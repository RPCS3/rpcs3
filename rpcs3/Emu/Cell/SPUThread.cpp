	...
	if ([&]()
	{
		...
		if (g_use_rtm) [[likely]]
		{
			...
			case umax:
			{
				...
				{
					perf_log.warning(u8"PUTLLC: took too long: %.3fus (%u c) (addr=0x%x) (S)", count2 / (utils::get_tsc_freq() / 1000'000.), count2, addr);
				}
			...
			default:
			{
				...
				{
					perf_log.warning(u8"PUTLLC: took too long: %.3fus (%u c) (addr = 0x%x)", count / (utils::get_tsc_freq() / 1000'000.), count, addr);
				}
...
void do_cell_atomic_128_store(u32 addr, const void* to_write)
{
...

	{
		...
		{
			perf_log.warning(u8"STORE128: took too long: %.3fus (%u c) (addr=0x%x)", result / (utils::get_tsc_freq() / 1000'000.), result, addr);
		}
