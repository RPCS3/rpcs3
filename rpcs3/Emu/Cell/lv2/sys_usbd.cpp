...
{
	auto& usbh = g_fxo->get<named_thread<usb_handler_thread>>();
	usbh.usb_hotplug_timeout = get_system_time() + 500'000ull;
	return 0;
}
#endif
...
void usb_handler_thread::operator()()
{
	...
	while (ctx && thread_ctrl::state() != thread_state::aborting)
	{
		...
		if (usb_hotplug_timeout && now > usb_hotplug_timeout)
		{
			...
			perform_scan();
			usb_hotplug_timeout = hotplug_supported ? 0 : get_system_time() + 4'000'000ull;
		}
