...
public:
	...
	u64 received_hotplug_event_timeout = 0;
...
{
	auto& usbh = g_fxo->get<named_thread<usb_handler_thread>>();
	usbh.received_hotplug_event_timeout = get_system_time() + 5'000'00ull;
	return 0;
}
#endif
...
void usb_handler_thread::perform_scan(bool force_scan)
{
	...
	{
		received_hotplug_event_timeout = 0;
		for (ssize_t index = 0; index < ndev; index++)
		{
			...
			const u64 usb_id = (static_cast<uint64_t>(desc.idVendor) << 48) | (static_cast<uint64_t>(desc.idProduct) << 32) | (port << 8) | address;
...
void usb_handler_thread::operator()()
{
	...
	while (ctx && thread_ctrl::state() != thread_state::aborting)
	{
		...
		if ((now > usb_hotplug_timeout && !hotplug_supported) || (received_hotplug_event_timeout && now > received_hotplug_event_timeout))
		{
