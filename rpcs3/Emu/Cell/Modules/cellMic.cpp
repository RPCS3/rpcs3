...
ALCdevice* microphone_device::open_device(std::string& name, u32 samplingrate, ALCenum num_al_channels, u32 buf_size)
{
	...
	{
		// Try to find a fitting device
		bool found_match = false;
		for (const std::string& device : enumerated_devices)
		{
			if (device.starts_with(name))
			{
				found_match = true;
				cellMic.warning("Capture device '%s' not in enumerated devices, but found similar device '%s'", name, device);
				cellMic.notice("Capture device '%s' renamed to '%s'", name, device);
				name = device;
				break;
			}
		}
