...
struct GLTraits
{
	...
	void recompile_fragment_program(const RSXFragmentProgram &RSXFP, fragment_program_type& fragmentProgramData, size_t ID)
	{
		...
		fs::file(fs::get_config_dir() + "dev_hdd1/shader/" + "FragmentProgram.glsl", fs::rewrite).write(fragmentProgramData.shader);
	}
	...
	void recompile_vertex_program(const RSXVertexProgram &RSXVP, vertex_program_type& vertexProgramData, size_t ID)
	{
		...
		fs::file(fs::get_config_dir() + "dev_hdd1/shader/" + "VertexProgram.glsl", fs::rewrite).write(vertexProgramData.shader);
	}
