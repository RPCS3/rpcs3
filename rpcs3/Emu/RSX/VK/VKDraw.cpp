...
void VKGSRender::update_draw_state()
{
	...
	if (REGS(0)->current_draw_clause.primitive >= rsx::primitive_type::points &&   // AMD/AMDVLK driver does not like it if you render points without setting line width for some reason
		REGS(0)->current_draw_clause.primitive <= rsx::primitive_type::line_strip)
	{
		const float actual_line_width =
			m_device->get_wide_lines_support() ? REGS(0)->line_width() * resolution_scaling_config.scale_factor() : 1.f;
		vkCmdSetLineWidth(*m_current_command_buffer, actual_line_width);
	}


	if (REGS(0)->blend_enabled_mask())
	{
	...
	if (REGS(0)->stencil_test_enabled())
	{
		const bool two_sided_stencil = REGS(0)->two_sided_stencil_test_enabled();
		VkStencilFaceFlags face_flag = (two_sided_stencil) ? VK_STENCIL_FACE_FRONT_BIT : VK_STENCIL_FRONT_AND_BACK;

		vkCmdSetStencilWriteMask(*m_current_command_buffer, face_flag, REGS(0)->stencil_mask());
		vkCmdSetStencilCompareMask(*m_current_command_buffer, face_flag, REGS(0)->stencil_func_mask());
		vkCmdSetStencilReference(*m_current_command_buffer, face_flag, REGS(0)->stencil_func_ref());

		if (two_sided_stencil)
		{
			vkCmdSetStencilWriteMask(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, REGS(0)->back_stencil_mask());
			vkCmdSetStencilCompareMask(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, REGS(0)->back_stencil_func_mask());
			vkCmdSetStencilReference(*m_current_command_buffer, VK_STENCIL_FACE_BACK_BIT, REGS(0)->back_stencil_func_ref());
		}
	}
	...
	if (REGS(0)->poly_offset_fill_enabled())
	{
		...
		const auto polygon_offset_scale = REGS(0)->poly_offset_scale();
		auto polygon_offset_bias = REGS(0)->poly_offset_bias();
	...
	if (m_device->get_depth_bounds_support())
	{
		...
		if (REGS(0)->depth_bounds_test_enabled())
		{
			...
			bounds_min = REGS(0)->depth_bounds_min();
			bounds_max = REGS(0)->depth_bounds_max();
		}
		else
		{
			// Avoid special case where min=max and depth bounds (incorrectly) fails
			bounds_min = std::min(0.f, REGS(0)->clip_min());
			bounds_max = std::max(1.f, REGS(0)->clip_max());
		}
...
bool VKGSRender::bind_texture_env()
{
	...
	for (u32 textures_ref = current_fp_metadata.referenced_textures_mask, i = 0; textures_ref; textures_ref >>= 1, ++i)
	{
		...
		if (REGS(0)->fragment_textures[i].enabled() &&
...
void VKGSRender::emit_geometry(u32 sub_index)
{
	...
	if (m_current_draw.subdraw_id == 0)
	{
		...
		const u64 alloc_size = REGS(0)->current_draw_clause.pass_count() * 168;
	...
	else
	{
		...
		else if (REGS(0)->current_draw_clause.is_single_draw())
		{
