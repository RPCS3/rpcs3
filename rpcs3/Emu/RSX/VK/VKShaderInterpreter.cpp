...
namespace vk
{
	...
	void shader_interpreter::preload(rsx::shader_loading_dialog* dlg)
	{
		...
		// Optimization - most drivers other than AMD don't have a severe enough penalty to force us to seed ALL formats here.
		if (vk::get_driver_vendor() != vk::driver_vendor::AMD)
		{
			// We could just remove this if other drivers have the same problem
			possible_color_formats.resize(1);
		}
