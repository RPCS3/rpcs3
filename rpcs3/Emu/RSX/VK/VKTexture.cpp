...
namespace vk
{
		...
		VkImageAspectFlags src_transfer_mask, VkImageAspectFlags dst_transfer_mask)
	{
		...
		VK_GET_SYMBOL(vkCmdCopyImage)(cmd, src->value, src->current_layout, dst->value, dst->current_layout, regions.size(), regions.data());
		...
		VkFilter filter)
	{
		...
		if (src->aspect() == VK_IMAGE_ASPECT_COLOR_BIT)
		{
			...
			VK_GET_SYMBOL(vkCmdBlitImage)(cmd, src->value, src->current_layout, dst->value, dst->current_layout, regions.size(), regions.data(), filter);
			return;
		}
			...
			VkImageAspectFlags aspect, VkImageAspectFlags transfer_flags = 0xFF)
		{
			...
			// 1. Copy unscaled to typeless surface
			VK_GET_SYMBOL(vkCmdCopyImageToBuffer)(cmd, src->value, src->current_layout, scratch_buf->value, 1, &src_copy_info);
			...
			VK_GET_SYMBOL(vkCmdCopyBufferToImage)(cmd, scratch_buf->value, typeless->value, VK_IMAGE_LAYOUT_GENERAL, 1, &copy_info);
			...
			VK_GET_SYMBOL(vkCmdCopyImageToBuffer)(cmd, typeless->value, VK_IMAGE_LAYOUT_GENERAL, scratch_buf->value, 1, &copy_info);
			insert_buffer_memory_barrier(cmd, scratch_buf->value, 0, scratch_size, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_PIPELINE_STAGE_TRANSFER_BIT, VK_ACCESS_TRANSFER_WRITE_BIT, VK_ACCESS_TRANSFER_READ_BIT);

			VK_GET_SYMBOL(vkCmdCopyBufferToImage)(cmd, scratch_buf->value, dst->value, dst->current_layout, 1, &dst_copy_info);
		};
