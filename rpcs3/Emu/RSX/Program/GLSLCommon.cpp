...
namespace glsl
{
	...
	void insert_glsl_legacy_function(std::ostream& OS, const shader_properties& props)
	{
		...
		if (props.require_texture_ops)
		{
			...
			"	vec4 convert;\n"
			"\n"
			"	if ((control_bits & SIGN_EXPAND_MASK) != 0)\n"
			"	{\n"
			"		// Expand to signed normalized\n"
			"		mask = uvec4(control_bits) & uvec4(EXPAND_R_MASK, EXPAND_G_MASK, EXPAND_B_MASK, EXPAND_A_MASK);\n"
			"		convert = (rgba * 2.f - 1.f);\n"
			"		return _select(rgba, convert, notEqual(mask, uvec4(0)));\n"
			"	}\n"
			"	else if ((control_bits & GAMMA_CTRL_MASK) != 0)\n"
			"	{\n"
			"		// Gamma correction\n"
			"		mask = uvec4(control_bits) & uvec4(GAMMA_R_MASK, GAMMA_G_MASK, GAMMA_B_MASK, GAMMA_A_MASK);\n"
