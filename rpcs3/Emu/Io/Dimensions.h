...
public:
	...
	void generate_random_number(u8* buf, u8 sequence, std::array<u8, 32>& reply_buf);
	void initialize_rng(u32 seed);
	void get_challenge_response(u8* buf, u8 sequence, std::array<u8, 32>& reply_buf);
	...
	void get_model(u8* buf, u8 sequence, std::array<u8, 32>& reply_buf);
	bool has_figure_been_added_removed() const;
	std::array<u8, 32> pop_added_removed_response();

	u16 get_figure(u8 index);
	bool remove_figure(u8 pad, u8 index, bool save);
	u16 load_figure(const std::array<u8, 0x2D * 0x04>& buf, fs::file in_file, u8 pad, u8 index);
    bool move_figure(u8 pad, u8 index, u8 old_pad, u8 old_index);
	// bool create_figure(fs::path pathName, u16 id);

protected:
	shared_mutex dimensions_mutex;
	std::array<dimensions_figure, 7> figures;

private:
	void random_uid(u8* uid_buffer);
	u8 generate_checksum(const std::array<u8, 32>& data,
		int num_of_bytes) const;
	std::array<u8, 8> decrypt(u8* buf);
	std::array<u8, 8> encrypt(u8* buf);
	u32 get_next();
