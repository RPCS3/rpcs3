...
void PadHandlerBase::get_next_button_press(const std::string& pad_id, const pad_callback& callback, const pad_fail_callback& fail_callback, bool get_blacklist, const std::vector<std::string>& buttons)
{
	...
	const auto preview_values = get_preview_values(data, buttons);
...
std::array<u32, PadHandlerBase::button::button_count> PadHandlerBase::get_mapped_key_codes(const std::shared_ptr<PadDevice>& device, const cfg_pad* cfg)
{
	...
	device->trigger_left  = FindKeyCode(button_list, cfg->l2);
	device->trigger_right = FindKeyCode(button_list, cfg->r2);
	device->axis_left[0]  = FindKeyCode(button_list, cfg->ls_left);
	device->axis_left[1]  = FindKeyCode(button_list, cfg->ls_right);
	device->axis_left[2]  = FindKeyCode(button_list, cfg->ls_down);
	device->axis_left[3]  = FindKeyCode(button_list, cfg->ls_up);
	device->axis_right[0] = FindKeyCode(button_list, cfg->rs_left);
	device->axis_right[1] = FindKeyCode(button_list, cfg->rs_right);
	device->axis_right[2] = FindKeyCode(button_list, cfg->rs_down);
	device->axis_right[3] = FindKeyCode(button_list, cfg->rs_up);
	...
	mapping[button::l2]       = device->trigger_left;
	mapping[button::l3]       = FindKeyCode(button_list, cfg->l3);
	mapping[button::r1]       = FindKeyCode(button_list, cfg->r1);
	mapping[button::r2]       = device->trigger_right;
	mapping[button::r3]       = FindKeyCode(button_list, cfg->r3);
	mapping[button::ls_left]  = device->axis_left[0];
	mapping[button::ls_right] = device->axis_left[1];
	mapping[button::ls_down]  = device->axis_left[2];
	mapping[button::ls_up]    = device->axis_left[3];
	mapping[button::rs_left]  = device->axis_right[0];
	mapping[button::rs_right] = device->axis_right[1];
	mapping[button::rs_down]  = device->axis_right[2];
	mapping[button::rs_up]    = device->axis_right[3];
