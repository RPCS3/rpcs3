...
namespace id_manager
{
	...
	struct id_traits_load_func
	{
		static constexpr pointer_keeper(*load)(utils::serial&) = [](utils::serial& ar) -> pointer_keeper
		{
			...
			{
				ptr = make_shared<T>(stx::launch_retainer{}, stx::exact_t<utils::serial&>(ar));
			}
			else
			{
				ptr = make_shared<T>(stx::exact_t<utils::serial&>(ar));
			}

			return [ptr](void* storage) { *static_cast<shared_ptr<T>*>(storage) = ptr; };
		};
	};
	...
	struct id_map
	{
		...
		std::array<atomic_ptr<T>, T::id_count> vec_data{};
		std::array<shared_ptr<T>, T::id_count> private_copy{};
		std::vector<id_key> vec_keys{};
		usz highest_index = 0;
...
class idm
{
	...
	template <typename T, typename RT>
	struct return_pair<shared_ptr<T>, RT>
	{
		shared_ptr<T> ptr;
		RT ret;
	...
	static id_manager::id_key* allocate_id(std::vector<id_manager::id_key>& vec, usz& highest_index, u32 type_id, u32 dst_id, u32 base, u32 step, u32 count, bool uses_lowest_id, std::pair<u32, u32> invl_range);
	...
	static shared_ptr<Type> create_id(F&& provider, u32 id = id_manager::id_traits<Type>::invalid)
	{
		static_assert(PtrSame<T, Type>, "Invalid ID type combination");
		...
		if (auto* key_ptr = allocate_id(map.vec_keys, map.highest_index, get_type<Type>(), id, traits::base, traits::step, traits::count, traits::uses_lowest_id, traits::invl_range))
		{
...
public:
	...
	static inline shared_ptr<Make> make_ptr(Args&&... args)
	{
		if (auto pair = create_id<T, Make>([&] { return make_shared<Make>(std::forward<Args>(args)...); }))
		{
			return shared_ptr<Make>(std::move(pair));
		}
	...
	template <typename T, typename Made = T, typename F>
		requires IdmBaseCompatible<T> && IdmCompatible<Made> && (std::is_invocable_v<F&&>)
	...
	template <typename T, typename Made = T>
		requires IdmBaseCompatible<T> && IdmCompatible<Made>
	static inline u32 import_existing(shared_ptr<Made> ptr, u32 id = id_manager::id_traits<Made>::invalid)
	{
	...
	template <typename T, typename Get = T>
		requires IdmBaseCompatible<T> && IdmCompatible<Get>
	...
	template <typename T, typename Get = T>
		requires IdmBaseCompatible<T> && IdmCompatible<Get>
	static inline shared_ptr<Get> get_unlocked(u32 id)
	{
		...
		return static_cast<shared_ptr<Get>>(found.first->load());
	}

	// Get the object, access object under reader lock
	template <typename T, typename Get = T, typename F, typename FRT = std::invoke_result_t<F, Get&>>
		requires IdmBaseCompatible<T> && IdmCompatible<Get>
	static inline std::conditional_t<std::is_void_v<FRT>, shared_ptr<Get>, return_pair<shared_ptr<Get>, FRT>> get(u32 id, F&& func)
	{
		const u32 index = get_index<Get>(id);
		...
		auto ptr =  static_cast<shared_ptr<Get>>(found.first->load());
	...
	static inline auto select(F&& func, Lock = std::true_type{})
	{
		...
		std::conditional_t<std::is_void_v<result_type>, u32, return_pair<shared_ptr<object_type>, result_type>> result{};
			...
			if (auto ptr = static_cast<object_type*>(id.observe()))
			{
				...
				if (sizeof...(Get) == 0 || ((key.type() == get_type<Get>()) || ...))
				{
					...
					else
					{
						...
						{
							result.ptr = static_cast<shared_ptr<object_type>>(id.load());
							break;
						}
					}
				}
	...
	template <typename T, typename Get = T, typename Ptr, typename Lock = std::true_type>
		requires IdmBaseCompatible<T> && IdmCompatible<Get> && std::is_convertible_v<Lock, bool>
	...
	template <typename T, typename Get = T, typename Lock = std::true_type>
		requires IdmBaseCompatible<T> && IdmCompatible<Get>
	static inline shared_ptr<Get> withdraw(u32 id, int = 0, Lock = std::true_type{})
	{
		shared_ptr<Get> ptr;
		{
			[[maybe_unused]] std::conditional_t<!!Lock(), std::lock_guard<shared_mutex>, const shared_mutex&> lock(id_manager::g_mutex);
			...
			if (const auto found = find_id<T, Get>(id); found.first)
			{
				ptr = static_cast<shared_ptr<Get>>(found.first->exchange(null_ptr));
				found.second->clear();
			}
		}
	...
	template <typename T, typename Get = T, typename F, typename FRT = std::invoke_result_t<F, Get&>>
		requires IdmBaseCompatible<T> && IdmCompatible<Get> && std::is_invocable_v<F, Get&>
	static inline std::conditional_t<std::is_void_v<FRT>, shared_ptr<Get>, return_pair<shared_ptr<Get>, FRT>> withdraw(u32 id, F&& func)
	{
		const u32 index = get_index<Get>(id);
		...
		if (const auto found = find_index<T, Get>(index, id); found.first)
		{
			...
			if constexpr (std::is_void_v<FRT>)
			{
				func(*_ptr);
				found.second->clear();
				return static_cast<shared_ptr<Get>>(found.first->exchange(null_ptr));
			}
			else
			{
				FRT ret = func(*_ptr);
				...
				if (ret)
				{
					...
					return {static_cast<shared_ptr<Get>>(found.first->load()), std::move(ret)};
				}

				found.second->clear();
				return {static_cast<shared_ptr<Get>>(found.first->exchange(null_ptr)), std::move(ret)};
			}
		}
