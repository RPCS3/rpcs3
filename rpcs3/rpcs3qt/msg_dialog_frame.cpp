...
void msg_dialog_frame::Create(const std::string& msg)
{
	...
	m_dialog->setWindowTitle(type.se_normal ? (u8"°T®§") : (u8"¿ù»~°T®§"));
	...
	if (type.button_type.unshifted() == CELL_MSGDIALOG_TYPE_BUTTON_TYPE_YESNO)
	{
		...
		m_button_yes = new QPushButton((u8"¬O(&Y)"), m_dialog);
		m_button_no = new QPushButton((u8"§_(&N)"), m_dialog);
	...
	if (qstr(msg).contains(u8"½sÄ¶ PPU ¼Ò²Õ"))
	{
