...
static QStringList getOptions(cfg_location location)
{
	...
	for (const auto& v : cfg_adapter::get_cfg(root, begin, end).to_list())
	{
...
EmuSettings::EmuSettings(const std::string& path) : QObject()
{
...
QComboBox* EmuSettings::CreateEnhancedComboBox(SettingsType type, QWidget* parent)
{
	...
	connect(box, static_cast<void(QComboBox::*)(int)>(&QComboBox::currentIndexChanged), [=](int index) {
		SetSetting(type, box->itemData(index).toString().toStdString());
	});

	return box;
}

QCheckBox* EmuSettings::CreateEnhancedCheckBox(SettingsType type, QWidget* parent)
{
...
std::vector<std::string> EmuSettings::GetLoadedLibraries()
{
	return currentSettings["Core"]["Load libraries"].as<std::vector<std::string>, std::initializer_list<std::string>>({});
}

void EmuSettings::SaveSelectedLibraries(const std::vector<std::string>& libs)
{
	currentSettings["Core"]["Load libraries"] = libs;
}

QStringList EmuSettings::GetSettingOptions(SettingsType type) const
{
