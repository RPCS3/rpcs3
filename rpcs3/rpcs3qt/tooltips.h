...
public:
	...
	const struct settings
	{
		...
		const QString enable_spu_events_busy_loop  = tr("Enable SPU RdEventStat spin.\nThis setting increases CPU usage.\nIt is beneficial for high-threaded CPUs (12+) with select number of games.");
		const QString enable_spu_reservation_busy_waiting_boost = tr("Enable SPU Reservation Busy Waiting Boost.\nThis setting increases CPU usage.\nIt is beneficial for mid or high end CPUs with select number of games.\nIt can cause severe performance degradation and stuttering on weak CPUs.");
		const QString disable_ppu_postponed_notification        = tr("Disable PPU Postponed Notification.\nThis setting can improve performance and/or fix graphical issues (e.g. flashing missing textures) on select number of games.");
