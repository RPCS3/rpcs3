...
SaveDataInfoDialog::SaveDataInfoDialog(QWidget* parent, const SaveDataInformation& info)
	: QDialog(parent)
{
	...
	connect(close_button, &QAbstractButton::clicked, this, &SaveDataInfoDialog::close);
...
SaveDataManageDialog::SaveDataManageDialog(QWidget* parent, unsigned int* sort_type, SaveDataEntry& save)
	: QDialog(parent)
{
	...
	// Events
	connect(pb_sort_action, &QAbstractButton::clicked, this, &SaveDataManageDialog::OnApplySort);
	connect(pb_copy, &QAbstractButton::clicked, this, &SaveDataManageDialog::OnCopy);
	connect(pb_delete, &QAbstractButton::clicked, this, &SaveDataManageDialog::OnDelete);
	connect(pb_info, &QAbstractButton::clicked, this, &SaveDataManageDialog::OnInfo);
	connect(pb_close, &QAbstractButton::clicked, this, &SaveDataManageDialog::close);
}
...
void SaveDataManageDialog::OnInfo()
{
...
void SaveDataManageDialog::OnDelete()
{
...
void SaveDataManageDialog::OnApplySort()
{	
...
SaveDataListDialog::SaveDataListDialog(QWidget* parent, bool enable_manage)
	: QDialog(parent)
{
	...
	if (!enable_manage)
	{
		...
		connect(m_select, &QAbstractButton::clicked, this, &SaveDataListDialog::OnSelect);
		hbox_action->addWidget(m_select);
		setWindowTitle(tr("Save Data Chooser"));
	}
	else {
		QPushButton *m_manage = new QPushButton(tr("&Manage"), this);
		connect(m_manage, &QAbstractButton::clicked, this, &SaveDataListDialog::OnManage);
...
void SaveDataListDialog::OnSort(int id)
{
	int idx = id;
	...
	if ((idx < m_sort_type_count) && (idx >= 0))
	{
		...
		// look at gamelistframe sortGameData for reference
	}
}
//Copy a existing save, need to get more arguments. maybe a new dialog.
void SaveDataListDialog::OnEntryCopy()
{
...
void SaveDataListDialog::OnEntryRemove()
{
	int idx = m_list->currentRow();
	if (idx != -1)
	{
...
void SaveDataListDialog::OnEntryInfo()
{
	int idx = m_list->currentRow();
	if (idx != -1)
	{
...
//Display info dialog directly.
void SaveDataListDialog::OnManage()
{
	int idx = m_list->currentRow();
	if (idx != -1)
	{
		...
		SaveDataManageDialog* manageDialog = new SaveDataManageDialog(this, &m_sort_type, save);
		manageDialog->setModal(true);
		manageDialog->show();
	}
}
//When you press that select button in the Chooser mode.
void SaveDataListDialog::OnSelect()
{
	int idx = m_list->currentRow();
	if (idx != -1)
	{
...
void SaveDataListDialog::ShowContextMenu(const QPoint &pos)
{
	...
	//Events
	connect(copyAct, &QAction::triggered, this, &SaveDataListDialog::OnEntryCopy);
	connect(removeAct, &QAction::triggered, this, &SaveDataListDialog::OnEntryRemove);
	connect(infoAct, &QAction::triggered, this, &SaveDataListDialog::OnEntryInfo);
