...
void skylander_dialog::load_skylander_path(u8 slot, const QString& path)
{
	...
	const u8 portal_slot = g_skyportal.load_skylander(data, std::move(sky_file), slot);
	update_edits();

	if (portal_slot == 0xFF)
	{
		QMessageBox::warning(this, tr("Failed to load the skylander!"), tr("Failed to load the skylander(%1)!\nSkylander %2 is already in use on the portal.").arg(path).arg(slot + 1), QMessageBox::Ok);
	}
}
