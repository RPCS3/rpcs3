...
bool ds3_pad_handler::Init()
{
	...
	for (ssize_t index = 0; index < cnt; index++)
	{
		...
		if (libusb_claim_interface(devhandle, 0) != LIBUSB_SUCCESS
			|| libusb_control_transfer(devhandle, LIBUSB_ENDPOINT_IN | LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE, 0x01, 0x03F2, 0, reportbuf, 64, 1000) < 0)
		{
			warn_about_drivers = true;
	...
	if (warn_about_drivers)
	{
		...
		LOG_ERROR(HLE, "[DS3] We recommend you use Zadig( https://zadig.akeo.ie/ ) to change your ds3 drivers to libusb-win32 ones");
	}
...
void ds3_pad_handler::send_output_report(const std::shared_ptr<ds3_device>& ds3dev)
{
	...
	libusb_control_transfer(ds3dev->handle, LIBUSB_ENDPOINT_OUT | LIBUSB_REQUEST_TYPE_CLASS | LIBUSB_RECIPIENT_INTERFACE, 0x09, 0x0201, 0, report_buf, sizeof(report_buf), 0);
}
...
bool ds3_pad_handler::get_data(const std::shared_ptr<ds3_device>& ds3dev)
{
	int num_bytes = 0;
	auto& dbuf = ds3dev->buf;

	if (libusb_interrupt_transfer(ds3dev->handle, 0x81, dbuf, 64, &num_bytes, 10) == LIBUSB_SUCCESS)
	{
