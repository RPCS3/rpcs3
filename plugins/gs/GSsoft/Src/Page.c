...
//	GS_LOG("getPixelAddress32: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable32[(y / 8) & 3][(x / 8) & 7];
	basepage = (bp >> 5) + ((y / 32) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 64 + columnTable32[y % 8][x % 8];
	if (word >= 0x100000) {
//		GS_LOG("getPixelAddress32: %d, %d, %x, %x -> %x\n", x, y, bp, bw, word);
		return 0;
	}
	return word;
}
...
//	GS_LOG("getPixelAddress16: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable16[(y / 8) & 7][(x / 16) & 3];
	basepage = (bp >> 5) + ((y / 64) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 128 + columnTable16[y % 8][x % 16];
/*	GS_LOG("block: %x\n", block);
	GS_LOG("basepage: %x\n", basepage);
	GS_LOG("word: %x\n", word);*/
	if (word >= 0x200000) return 0;
	return word;
}
...
//	GS_LOG("getPixelAddress16: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable16S[(y / 8) & 7][(x / 16) & 3];
	basepage = (bp >> 5) + ((y / 64) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 128 + columnTable16[y % 8][x % 16];
/*	GS_LOG("block: %x\n", block);
	GS_LOG("basepage: %x\n", basepage);
...
//	GS_LOG("getPixelAddress8: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable8[(y / 16) & 3][(x / 16) & 7];
	basepage = (bp >> 5) + ((y / 64) * ((bw+127)/128)) + x/128; 
	word = ((basepage * 32) + block) * 256 + columnTable8[y % 16][x % 16];
//	GS_LOG("block: %x\n", block);
//	GS_LOG("basepage: %x\n", basepage);
//	GS_LOG("word: %x\n", word);
	if (word >= 0x400000) return 0;
	return word;
}
...
u32 getPixelAddress4(int x, int y, u32 bp, u32 bw) {
	...
	block = (bp & 0x1f) + blockTable4[(y / 16) & 7][(x / 32) & 3];
	basepage = (bp >> 5) + ((y / 128) * ((bw+127)/128)) + x/128; 
	word = ((basepage * 32) + block) * 512 + columnTable4[y % 16][x % 32];
	if (word >= 0x800000) return 0;
	return word;
}
...
//	GS_LOG("getPixelAddress32: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable32Z[(y / 8) & 3][(x / 8) & 7];
	basepage = (bp >> 5) + ((y / 32) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 64 + (y % 8) * 8 + x % 8;
	if (word >= 0x100000) return 0;
	return word;
}
...
//	GS_LOG("getPixelAddress16: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable16Z[(y / 8) & 7][(x / 16) & 3];
	basepage = (bp >> 5) + ((y / 64) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 128 + (y % 8) * 16 + x % 16;
/*	GS_LOG("block: %x\n", block);
	GS_LOG("basepage: %x\n", basepage);
	GS_LOG("word: %x\n", word);*/
	if (word >= 0x200000) return 0;
...
//	GS_LOG("getPixelAddress16: %d, %d, %x, %x\n", x, y, bp, bw);
	block = (bp & 0x1f) + blockTable16SZ[(y / 8) & 7][(x / 16) & 3];
	basepage = (bp >> 5) + ((y / 64) * (bw/64)) + x/64; 
	word = ((basepage * 32) + block) * 128 + (y % 8) * 16 + x % 16;
/*	GS_LOG("block: %x\n", block);
	GS_LOG("basepage: %x\n", basepage);
