// The code has been designed for 64Mb flash and uses as file support flash.ps2

#include <stdio.h>
#include "DEV9.h"

//#define DEV9_FLASH_LOG DEV9_LOG
...
void CALLBACK FLASHinit(){
	...
	if (fd=fopen("memcards\\flash.ps2", "rb")){
		fread(file, 1, CARD_SIZE_ECC, fd);
		fclose(fd);
	}else{
		memset(file, 0xFF, CARD_SIZE_ECC);
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("Working without a file. Copy mcd002.ps2 to flash.ps2\n");
#endif
	}
}
...
u32  CALLBACK FLASHread32(u32 addr, int size) {
	...
	case FLASH_R_CMD:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH CMD %dbit read %s DENIED\n", size*8, getCmdName(cmd));
#endif
		return cmd;

	case FLASH_R_ADDR:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH ADDR %dbit read DENIED\n", size*8);
#endif
		return 0;
	
	case FLASH_R_CTRL:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH CTRL %dbit read 0x%08lX\n", size*8, ctrl);
#endif
		return ctrl;
...
#ifdef DEV9_FLASH_LOG
			DEV9_FLASH_LOG("*FLASH ID %dbit read 0x%08lX\n", size*8, id);
#endif
...
#ifdef DEV9_FLASH_LOG
			DEV9_FLASH_LOG("*FLASH STATUS %dbit read 0x%08lX\n", size*8, value);
#endif
			return value;
		}//else fall off
	...
	default:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH Unkwnown %dbit read at address %lx\n", size*8, addr);
#endif
		return 0;
	}
}
...
void CALLBACK FLASHwrite32(u32 addr, u32 value, int size) {
...
#ifdef DEV9_FLASH_LOG
				DEV9_FLASH_LOG("*FLASH CMD %dbit write %s ILLEGAL in busy mode - IGNORED\n", size*8, getCmdName(value));
#endif
				break;
			}
		}
...
#ifdef DEV9_FLASH_LOG
				DEV9_FLASH_LOG("*FLASH CMD %dbit write %s ILLEGAL after WRITEDATA cmd - IGNORED\n", size*8, getCmdName(value));
#endif
...
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH CMD %dbit write %s\n", size*8, getCmdName(value));
#endif
	...
	case FLASH_R_ADDR:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH ADDR %dbit write 0x%08lX\n", size*8, value);
#endif
		address |= (value & 0xFF) << (addrbyte == 0 ? 0 : (1 + 8 * addrbyte));
		addrbyte++;
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH ADDR = 0x%08lX (addrbyte=%d)\n", address, addrbyte);
#endif
		if (!(value & 0x100)){	// address is complete
...
#ifdef DEV9_FLASH_LOG
			{
				...
				DEV9_FLASH_LOG("*FLASH ADDR = 0x%08lX (%d:%d:%d) (addrbyte=%d) FINAL\n", address, blocks, pages, bytes, addrbyte);
			}
#endif
		}
		break;
	...
	case FLASH_R_CTRL:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH CTRL %dbit write 0x%08lX\n", size*8, value);
#endif
	...
	case FLASH_R_ID:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH ID %dbit write 0x%08lX DENIED :P\n", size*8, value);
#endif
		break;

	default:
#ifdef DEV9_FLASH_LOG
		DEV9_FLASH_LOG("*FLASH Unkwnown %dbit write at address 0x%08lX= 0x%08lX IGNORED\n", size*8, addr, value);
#endif
		break;
	}
}
