...
namespace fs
{
	...
	[[noreturn]] void xnull(const std::source_location& src_loc);
	[[noreturn]] void xfail(const std::source_location& src_loc);
	[[noreturn]] void xovfl();
	...
	public:
		...
		bool trunc(u64 length, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		// Get file information
		stat_t stat(const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
			return m_file->stat();
		}
		...
		// Sync file buffers
		void sync(const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
			return m_file->sync();
		}
		...
		u64 read(void* buffer, u64 count, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		u64 write(const void* buffer, u64 count, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		u64 seek(s64 offset, seek_mode whence = seek_set, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		// Get file size
		u64 size(const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
			return m_file->size();
		}
		...
		u64 pos(const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		const file& write(const std::basic_string<T>& str, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		const file& write(const T& data, pod_tag_t = pod_tag, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		const file& write(const std::vector<T>& vec, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		bool read(std::basic_string<T>& str, usz _size = umax, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		bool read(T& data, pod_tag_t = pod_tag, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		// Read POD std::vector
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		bool read(std::vector<T>& vec, usz _size = umax, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
		...
		template <typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		T read(pod_tag_t = pod_tag, const std::source_location src_loc = std::source_location::current()) const
		{
			T result;
		...
		std::basic_string<T> to_string(const std::source_location src_loc = std::source_location::current()) const
		{
			std::basic_string<T> result;
			result.resize(size() / sizeof(T));
			if (seek(0), !read(result, result.size(), src_loc)) xfail(src_loc);
			return result;
		}

		// Read full file to std::vector
		template<typename T> requires (std::is_trivially_copyable_v<T> && !std::is_pointer_v<T>)
		std::vector<T> to_vector(const std::source_location src_loc = std::source_location::current()) const
		{
			std::vector<T> result;
			result.resize(size() / sizeof(T));
			if (seek(0), !read(result, result.size(), src_loc)) xfail(src_loc);
		...
		// Gathered write
		u64 write_gather(const iovec_clone* buffers, u64 buf_count, const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_file) xnull(src_loc);
	...
	public:
		...
		bool read(dir_entry& out, const std::source_location src_loc = std::source_location::current()) const
		{
		...
		void rewind(const std::source_location src_loc = std::source_location::current()) const
		{
			if (!m_dir) xnull(src_loc);
			return m_dir->rewind();
		}
