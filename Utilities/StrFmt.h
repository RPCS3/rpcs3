...
namespace fmt
{
	...
	[[noreturn]] void raw_throw_exception(const std::source_location&, const char*, const fmt_type_info*, const u64*);

	// Throw exception with formatting
	template <typename CharT, usz N, typename... Args>
	struct throw_exception
	{
		[[noreturn]] FORCE_INLINE SAFE_BUFFERS() throw_exception(const CharT(&fmt)[N], const Args&... args, const std::source_location src_loc = std::source_location::current())
		{
	...
	decltype(auto) ensure(T&& arg, const CharT(&fmt)[N], tie<Args...> args, const std::source_location src_loc = std::source_location::current()) noexcept
	{
