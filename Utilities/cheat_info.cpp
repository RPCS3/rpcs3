...
bool cheat_info::from_str(const std::string& cheat_line)
{
	...
	s64 val64 = 0;
	if (cheat_vec.size() != 6 || !cfg::try_to_int64(&val64, cheat_vec[2], 0, cheat_type_max - 1))
	{
	...
	apply_on_boot = cheat_vec[5] == "true";
...
{
	std::string cheat_str = game + "@@@" + description + "@@@" + std::to_string(static_cast<u8>(type)) + "@@@" + std::to_string(offset) + "@@@" + red_script + "@@@" + (apply_on_boot ? "true" : "false") + "@@@";
	return cheat_str;
}
